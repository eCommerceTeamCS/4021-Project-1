...
The PACF plot for the residuals of the NO2.trend.seasonal shows sinusoidal decay. However, it's cutoff seems to be around 3, so we will test with autoregressive components up to 3. 
Because the ACF and PACF both show sinusoidal decay, we will also test some ARMA models with both autoregressive and moving average components.
Then we will calculate AIC values to assess several model choices. 
...
Note: we have hidden the code for the creationi of the varma  model and AIC matrix due to its extensive output. (using include=FALSE)
...
*Note* we have omitted the code for the creation of the varma models due to the excess output. (using include=FALSE)
