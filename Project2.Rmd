...
# The test data frame
```{r}
next.7day.CO <- data.frame(time.temp = 366:384, CO = CO.ts[366:384])
next.7day.NO2 <- data.frame(time.temp = next.7day.time, NO2 = NO2.ts[next.7day.time])
```
...
```{r}
E_Y.pred.Tmin <- predict(Tmin.lm, newdata=next.6mo.Tmin)
e_t.pred.Tmin <- forecast(e.Tmin.lm, h=6)
next.6mo.prediction.Tmin <- E_Y.pred.Tmin + e_t.pred.Tmin$mean

E_Y.pred.Tmax <- predict(Tmax.lm, newdata=next.6mo.Tmax)
e_t.pred.Tmax <- forecast(e.Tmax.lm, h=6)
next.6mo.prediction.Tmax <- E_Y.pred.Tmax + e_t.pred.Tmax$mean
```
...
par(mfrow=c(1,2))
# Tmin forecasts
plot(ts(next.6mo.Tmin$Tmin),type='o',ylim=c(20,65))
lines(ts(next.6mo.prediction.Tmin),col='red',type='o')
lines(1:6, E_Y.pred.Tmin + e_t.pred.Tmin$lower[,2], col = "red", lty = "dashed")
lines(1:6, E_Y.pred.Tmin + e_t.pred.Tmin$upper[,2], col = "red", lty = "dashed")
legend(1,60, legend = c("Actual", "Predicted"), lwd = 2, col = c("black", "red"))

#Tmax forecasts
plot(ts(next.6mo.Tmax$Tmax),type='o',ylim=c(40,85))
lines(ts(next.6mo.prediction.Tmax),col='red',type='o')
lines(1:6, E_Y.pred.Tmax + e_t.pred.Tmax$lower[,2], col = "red", lty = "dashed")
lines(1:6, E_Y.pred.Tmax + e_t.pred.Tmax$upper[,2], col = "red", lty = "dashed")
