...
Finally, the Ljung-Box statistic shows that the p-values do not touch 0, and is at adequate at all the lags.
All of these attributes from the diagnotics plots indicate a valid model. Now, let's look at the model with the best AIC, ARMA(1,3).
```{r}
...
lines(dailyAQ$NO2.GT.[1:365] + allResiduals$NO2[1:365],col="green")}
...
Multivariate: CO
```{r}
{plot(COmean + T.simM$series[,1], type='l')
lines(dailyAQ$CO.GT.[1:365] + allResiduals$CO[1:365],col="blue")}
```
Above is a plot of ? which shows ___
...
Multivariate: NO2
```{r}
{plot(NO2mean + T.simM$series[,2], type='l')
